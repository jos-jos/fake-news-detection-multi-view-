...
def read_imdb(path='./datasets/style_dataset.pkl', split=0.8):
    '''
    reviews, labels = [], []
    # 创建分词器
    ...
    tokenizer = get_tokenizer('basic_english')
    split_num = int(len(data)*split)
    train_sets = data[:split_num]
    test_sets = data[split_num:]
    train_review = [tokenizer(i[0]) for i in train_sets]
    train_label = [i[1] for i in train_sets]
    test_review = [tokenizer(i[0]) for i in test_sets]
    test_label = [i[1] for i in test_sets]
    return train_review, train_label, test_review, test_label

# 对数据集中句子的单词长度进行统一
def build_dataset(reviews, labels, vocab, max_len=256):
    ...
    print(len(reviews))
    print(text_transform(reviews).shape)
    print(len(labels))
